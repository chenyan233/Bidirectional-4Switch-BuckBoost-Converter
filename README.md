...
上半部分给出两个半桥、33 µH 主电感和串联分流器的连接关系，电感电流以 A→B 为正方向。下半部分将 STM32 双闭环、PWM 硬件门控、HIP4081AIBZ 栅极驱动以及电压、电流采样分开表示；过流、过压故障经比较器和锁存逻辑直接进入硬件关断路径，不依赖软件闭环响应。图中开关采用理想化符号，端口保护、体二极管和辅助电源未展开。
...
带帽变量表示滤波后的测量值；$e_v$、$e_i$ 分别为电压、电流误差，$i_{L,raw}^{*}$ 为限幅前电流参考，$m$ 为工作区信号，$s_{DCM}$ 为导通模式控制信号，$EN$ 为图示硬件允许信号，$g_{1\ldots4}$ 为四路开关控制信号。同一信号在输出端和输入端重复标注，方便沿反馈路径追踪。反向运行时需改为调节 A 端电压，并一致处理电流参考与反馈的符号；该图表示控制功能关系，不等同于已完成的固件或硬件验证。
...
上半部分以 A→B、理想 CCM 稳态为例，对比三个工作区。降压区由 A 桥调制、Q3 保持导通；升压区由 B 桥调制、Q1 保持导通；交接区由两个桥臂协调调制。图中 $d_1$、$d_3$ 分别表示 Q1、Q3 的高侧导通占空比，因此交接区内 $d_1=D_{buck}$、$d_3=1-D_{boost}$，不能把 $d_3$ 直接当成 $D_{boost}$。
...
本工况由 B 端向 A 端供能，B 端标称电压为 24 V，A 端目标电压为 18 V。曲线名称、单位及电感电流正方向与图 9 相同。A 端电压在前段下降至约 10 V，随后恢复至约 18 V；负向电感电流对应 B→A 的能量传输。因此，本图用于展示反向降压的动态过程，不能据此认定全过程已经满足稳压精度要求。
